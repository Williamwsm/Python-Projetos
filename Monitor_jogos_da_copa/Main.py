...
def get_match_data():
    return requests.get(url='https://temporeal.lance.com.br/storage/matches/copa-do-mundo-2022-28-11-2022-brasilxsuica.json'
    ).json()
...
def show_history():
    ...
    for narration in narrations:
        ...
        if len (sys.argv)> sys.argv[1] == 'history':
            show_history()
        ...
        while True:
            ...
            narrations = match_data['match']['narrations']
            last_narration = narration[len(narrations)-1]
            last_narration_time = datetime.strftime(last_narration['created_at'],'%Y-%m-%dT%H:%M:%S.000000Z' )
